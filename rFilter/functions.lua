...
local function SetExtraValue(button,extravalue)
  if extravalue and extravalue > 1 then
    button.extravalue:SetText(extravalue)
  else
...
local function UpdateCooldown(button)
  ...
  local currentCharges, _, _, _, _ = GetSpellCharges(button.settings.spellid)
  SetExtraValue(button,currentCharges)
